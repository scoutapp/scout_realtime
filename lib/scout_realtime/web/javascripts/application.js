  ...
  update:function(processes){
    ...
    for(cmd in Processes.index) {
      ...
      $.each(['count', 'cpu', 'memory'], function(index, metric) {
        ...
        graph.data.push({time: new Date().getTime(), value: [data[metric]], tooltip: data[metric]});
        graph.refresh();
        graph.data.shift();
        $(graph.element).siblings('.latest_value').text(data[metric]);
      });
    ...
    $row.find(".chart").each(function(i){
      ...
      if(bufferedData.length > 0) {
        ...
        $.each(bufferedData, function(data_index, value) {
          data.push({time: startTime + data_index * 1000, value: [value], tooltip: value})
        });

		    chart = new lineChart({ element: $(this).get(0), data: data });
        chart.draw();
      } else {
