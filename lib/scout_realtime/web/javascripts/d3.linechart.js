function lineChart(params) {
  ...
  this.draw = function() {
	...
	this.refresh = function() {
		var now = new Date();
		// update domains
		x.domain([now - 1*60*1000, now.getTime()]);
  	y.domain([yMin(this.data),yMax(this.data)]);
      ...
      .attr("transform", null)	
			...
			.transition()
					.duration(950) // duration is the same as the update interval...if it equals the update interval, it gets canceled?
      		.ease("linear")
					.attr("transform", "translate(" + x(x.domain()[0]-1000) + ")")
	}
