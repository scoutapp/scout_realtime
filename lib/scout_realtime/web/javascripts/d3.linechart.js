function lineChart(params) {
  ...
  this.draw = function() {
    ...
    var width = (selection.node().clientWidth || parseInt($(selection.node()).css('width'))) - margin.left - margin.right;
    ...
    if(type == 'overview') {
      xAxis = selection.append("g")
        ...
        .call(x.axis = d3.svg.axis().scale(x).ticks(6).orient("bottom"));
    ...
    if(window.refresher) {
      ...
      chartSection.enter()
          ...
          .attr("width", barWidth)
          .attr("height", height)
          .attr("transform", function(d, i) { return "translate(" + (x(d.time) - barWidth) + "," + margin.top + ")"; })
          .on("mouseover", function(d) {
            chartInfo.html(_this.tooltip(d));
          })
          .on("mouseout", function(d) {
