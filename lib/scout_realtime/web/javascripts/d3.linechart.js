function lineChart(params) {
  ...
  this.draw = function() {
    ...
    var width = (selection.node().clientWidth || parseInt($(selection.node()).css('width'))); // don't take margins into account for smoothness' sake
    ...
    if(type == 'overview') {
      ...
      yAxis = selection.append("g")
        .attr("class", 'y axis')
        .attr("transform", "translate(" + margin.left + "," + margin.top + ")")
        .call(y.axis = d3.svg.axis().scale(y).ticks(1).orient("left"));
    }
